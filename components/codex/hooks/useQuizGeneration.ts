...
function generateDistractors(answer: string, terms: string[], count: number = 3): string[] {
  ...
  // Filter terms that aren't the answer and are similar in length
  const candidates = terms.filter(t => {
    const n = t.toLowerCase().trim()
    return n !== normalized &&
           n.length > 2 &&
           Math.abs(t.length - answer.length) < answer.length // Similar length
  })
  ...
  const generate = useCallback(async (
    ...
    try {
      ...
      if (types.includes('multiple_choice') && definitions.length > 0) {
        allQuestions.push(...generateMultipleChoice(definitions, terms))
        await yieldToMain()
      }
