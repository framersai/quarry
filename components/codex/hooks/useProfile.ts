...
function calculateLevel(xp: number): { 
  ...
  let xpToNextLevel = LEVEL_THRESHOLDS[1]?.xpRequired || 100
  ...
  for (let i = LEVEL_THRESHOLDS.length - 1; i >= 0; i--) {
    if (xp >= LEVEL_THRESHOLDS[i].xpRequired) {
      ...
      xpToNextLevel = LEVEL_THRESHOLDS[i + 1]?.xpRequired || xp + 1000
      break
    }
  }

  const progress = (xp - xpForCurrentLevel) / (xpToNextLevel - xpForCurrentLevel)
...
export function useProfile(options: UseProfileOptions = {}) {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      const xp = await profileStorage.get('totalXp', 0)
  ...
  const saveProfile = useCallback(async () => {
    try {
      ...
      await profileStorage.set('totalXp', totalXp)
  ...
  }, [profileId, displayName, avatar, bio, totalXp, featuredAchievements, stats, achievements, activityHeatmap, subjectProficiency, settings])
   ...
   */
  const addXp = useCallback(async (amount: number): Promise<{
    newTotal: number
    leveledUp: boolean
    newLevel?: number
    newTitle?: string
    ...
    setTotalXp(newTotal)
    await profileStorage.set('totalXp', newTotal)
  ...
  const recordStudySession = useCallback(async (session: StudySession) => {
    ...
    await progressStorage.set('lastActiveAt', new Date().toISOString())
  }, [activityHeatmap, stats, updateStats, addXp])
  ...
  return {
    ...
    xpToNextLevel: levelData.xpToNextLevel - totalXp,
    streakStatus,
