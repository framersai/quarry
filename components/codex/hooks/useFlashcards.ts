...
export function useFlashcardGeneration() {
  ...
  const generateFromKeywords = useCallback(async (
    ...
    try {
      ...
      // Prioritize topics > people > places > nouns
      const allTerms = [...new Set([...topics, ...people, ...places, ...nouns.slice(0, 15)])]

      // Find sentences containing each term
  ...
  const generateFromDefinitions = useCallback(async (
    ...
    try {
      ...
      const definitionPatterns = [
        ...
        /\*\*([^*]+)\*\*[:\s]+(.+?)[.!?]/g, // **Term**: definition pattern
        /`([^`]+)`\s+(?:is|are|refers to)\s+(.+?)[.!?]/g, // `term` is definition pattern
      ]
  ...
  const generateAll = useCallback(async (
    content: string,
    strandSlug: string,
    options: { forceRegenerate?: boolean; useLLM?: boolean } = {}
  ): Promise<{
    cards: Omit<Flashcard, 'id' | 'fsrs' | 'createdAt' | 'updatedAt' | 'suspended' | 'starred'>[]
    stats: { keywords: number; definitions: number; skipped: number; invalid: number }
    fromCache: boolean
  }> => {
    ...
    const keywordCards = await generateFromKeywords(content, strandSlug, existingHashes)
    const definitionCards = await generateFromDefinitions(content, strandSlug, existingHashes)

    // Combine and deduplicate between methods
    const allCards: typeof keywordCards = []
    ...
    if (allCards.length > 0) {
      ...
      console.log(`[FlashcardGeneration] Cached ${allCards.length} cards for ${strandSlug}`)
