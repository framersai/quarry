...
import { 
  ...
  Filter, ChevronDown, Sparkles, RefreshCw 
} from 'lucide-react'
...
export default function TemplateSelector({
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (hiddenTemplates.length > 0) {
          filtered = filtered.filter(t => !hiddenTemplates.includes(t.id))
        }
    ...
    load()
  }, [allowedCategories, hiddenTemplates])
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-zinc-500">Loading templates...</span>
      </div>
    )
  }
