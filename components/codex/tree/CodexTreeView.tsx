...
import { CodexTreeNode, transformToArboristData, cloneTree, findNodeById, MoveOperation } from './types'
...
function WeaveNode({
  ...
  // Handle recursive selection
  const handleSelectRecursive = useCallback(async () => {
    if (!onSelectRecursive) return
    const allPaths = [node.data.path, ...getAllDescendantPaths(node.data)]
    await onSelectRecursive(node.data.path, allPaths)
  }, [node.data, onSelectRecursive])
    ...
    <div ref={dragHandle} style={style} className="group/weave flex items-center">
      ...
      {selectionMode && onToggleSelection && (
        <SmartSelectionCheckbox
          ...
          onSelectSingle={handleSelectRecursive} // Default click = recursive select all strands
          onSelectRecursive={handleSelectRecursive}
          isDark={isDark}
          className="ml-0.5"
        />
      )}
      ...
      <button
        onClick={() => node.toggle()} // Always toggle/navigate, checkbox handles selection
        className={`
          flex-1 flex items-center gap-1.5 rounded-lg px-1.5 py-1 text-left min-w-0
          ${isSelected 
            ? 'bg-gradient-to-r from-purple-100 to-indigo-100 dark:from-purple-900/40 dark:to-indigo-900/40 border border-purple-300 dark:border-purple-700' 
            : 'hover:bg-purple-50 dark:hover:bg-purple-900/20 border border-transparent'
          }
      ...
      >
        ...
        {enableDragDrop && !isMobile && (
          <GripVertical className="w-2.5 h-2.5 flex-shrink-0 cursor-grab text-transparent group-hover/weave:text-purple-400 transition-colors" />
        )}
        
        {/* Icon */}
        {customStyle?.emoji ? (
          <span className="text-sm flex-shrink-0">{customStyle.emoji}</span>
        ) : customStyle?.icon && isValidIconName(customStyle.icon) ? (
          <div className={`p-0.5 rounded bg-gradient-to-br ${palette.gradient} flex-shrink-0`}>
            <DynamicIcon name={customStyle.icon} className="w-3 h-3 text-white" />
          </div>
        ) : (
          <div className={`p-0.5 rounded bg-gradient-to-br ${palette.gradient} flex-shrink-0`}>
        ...
        {isEditing ? (
          ...
          <span className="flex-1 text-[11px] font-bold truncate capitalize text-zinc-900 dark:text-zinc-100 min-w-0">
            {displayName}
          </span>
        )}
        
        {/* Stats - compact */}
        <span className="flex items-center gap-0.5 text-[8px] text-zinc-500 dark:text-zinc-400 flex-shrink-0">
          <Box className="w-2 h-2 text-amber-500" />
          {loomCount}
        </span>
        <span className="flex items-center gap-0.5 text-[8px] text-zinc-500 dark:text-zinc-400 flex-shrink-0">
    ...
    >
      ...
      >
        ...
        {isEditing ? (
          ...
          <span className={`flex-1 text-[11px] font-semibold truncate capitalize min-w-0 ${customStyle?.darkText ? 'text-zinc-900' : 'text-zinc-800 dark:text-zinc-100'}`} style={customStyle?.textColor ? { color: customStyle.textColor } : undefined}>
            {displayName}
          </span>
        )}
        
        {/* Strand count */}
        <span className={`text-[8px] font-bold px-1 py-0.5 rounded flex-shrink-0 ${customStyle?.accentColor ? 'text-white' : 'bg-amber-200 dark:bg-amber-800 text-amber-700 dark:text-amber-200'}`} style={customStyle?.accentColor ? { backgroundColor: customStyle.accentColor } : undefined}>
          {node.data.strandCount}
        </span>
...
function StrandNode({
  ...
  // Handle recursive selection
  const handleSelectRecursive = useCallback(async () => {
    if (!onSelectRecursive || !isDir) return
    const allPaths = [node.data.path, ...getAllDescendantPaths(node.data)]
    await onSelectRecursive(node.data.path, allPaths)
  }, [node.data, onSelectRecursive, isDir])
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => {
          // Always navigate/open - checkbox handles selection separately
          if (isDir) {
            node.toggle()
            onNavigate?.(node.data.path) // Navigate to directory
          } else {
            // For files, just select - the onSelect callback handles file opening
            tree.select(node.id)
          }
        }}
        className={`
          flex-1 flex items-center gap-0.5 rounded px-1 py-0.5 text-left min-w-0
          ${isSelected
            ...
            : isDir
              ...
              : 'hover:bg-emerald-50/70 dark:hover:bg-emerald-900/20 border border-transparent'
          }
          ${node.isDragging ? 'opacity-50' : ''}
      ...
      >
        ...
        {isEditing ? (
          ...
          <span className={`flex-1 text-[11px] font-medium truncate capitalize leading-tight min-w-0 ${
            isSelected
        ...
        {isDir && node.data.strandCount > 0 && !isEditing && (
          <span className="text-[8px] font-semibold px-0.5 py-0.5 rounded bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 flex-shrink-0">
            {node.data.strandCount}
          </span>
        )}
  ...
  return (
    ...
    >
      <button
        onClick={() => node.toggle()}
        className={`
          w-full flex items-center gap-1.5 rounded px-2 py-0.5 text-left
          ${isSelected
      ...
      >
        ...
        {isEditing ? (
          ...
          <span className="flex-1 text-[11px] font-medium text-zinc-600 dark:text-zinc-300 truncate">
            {displayName}
          </span>
        )}
        
        {node.data.strandCount > 0 && (
          <span className="text-[9px] font-semibold px-1 py-0.5 rounded bg-zinc-200 dark:bg-zinc-800 text-zinc-500">
            {node.data.strandCount}
          </span>
        )}
      </button>
...
export default function CodexTreeView({
  ...
  const treeData = useMemo(() => transformToArboristData(data), [data])
  ...
  return (
    <div className="h-full flex flex-col space-y-2">
      ...
      {(totalStrands > 0 || totalWeaves > 0) && (
        <div className="rounded border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/40 px-3 py-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-zinc-500 dark:text-zinc-400">{totalWeaves} weaves</span>
            </div>
