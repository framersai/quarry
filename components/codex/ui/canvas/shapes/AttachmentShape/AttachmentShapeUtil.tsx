...
export class AttachmentShapeUtil extends BaseBoxShapeUtil<AttachmentShape> {
   ...
   */
  override onResize: TLOnResizeHandler<AttachmentShape> = (shape, info) => {
   ...
   * Export shape as SVG
   */
  override toSvg(shape: AttachmentShape) {
    ...
    icon.setAttribute('x', (w / 2).toString())
    icon.setAttribute('y', (h / 2 - 10).toString())
    ...
    nameText.setAttribute('x', (w / 2).toString())
    nameText.setAttribute('y', (h / 2 + 20).toString())
    nameText.setAttribute('font-size', '12')
    nameText.setAttribute('text-anchor', 'middle')
    nameText.setAttribute('fill', '#166534')
    nameText.textContent = fileName.length > 20 ? fileName.slice(0, 17) + '...' : fileName
    g.appendChild(nameText)

    // File size
    const sizeText = document.createElementNS('http://www.w3.org/2000/svg', 'text')
    sizeText.setAttribute('x', (w / 2).toString())
    sizeText.setAttribute('y', (h / 2 + 36).toString())
