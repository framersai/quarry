/**
 * Advanced search bar with filter options for FABRIC Codex
...
import { 
  ...
  ChevronDown, GraduationCap, Folder, Layers, Plus, CheckCircle, RotateCcw
} from 'lucide-react'
...
export default function SearchBar({
  ...
  const filteredTags = useMemo(() => {
    if (!tagInput) return availableTags.slice(0, 10)
    const lower = tagInput.toLowerCase()
    return availableTags.filter(t => t.toLowerCase().includes(lower)).slice(0, 10)
    ...
    <div className="space-y-1.5">
      ...
      >
        ...
        <input
          ...
          `}
          aria-label="Search FABRIC Codex"
          inputMode="search"
        />

        {/* Action Buttons */}
        <div className={`absolute top-1/2 transform -translate-y-1/2 flex items-center ${compact ? 'right-1 gap-0.5' : 'right-2 gap-1'}`}>
          {/* Clear Query Button */}
          <AnimatePresence>
            {options.query && (
              <motion.button
                ...
                <>
                  ...
                  >
                    <span className="flex items-center gap-2 font-medium">
                      ...
                      Advanced Filters
                      {((options.difficulty) || (options.tags && options.tags.length > 0) || (options.subjects && options.subjects.length > 0)) && (
