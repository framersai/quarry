...
import type { WorthinessSignals } from '@/components/codex/types'
...
function SuggestedTagItem({
  ...
  return (
    <div className="group flex items-start gap-2 p-1.5 rounded-lg hover:bg-zinc-100/50 dark:hover:bg-zinc-800/50 transition-colors">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <TagPill tag={tag.tag} variant="suggested" />
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <button
          onClick={onReject}
          className="p-1 rounded-full text-rose-500 hover:bg-rose-500/20 transition-colors"
...
export default function BlockTagsDisplay({
  ...
  const hasSuggested = block.suggestedTags.length > 0
  ...
  const visibleTags = useMemo(() => {
    ...
    return block.tags.slice(0, maxVisibleTags)
  }, [block.tags, isExpanded, mode, maxVisibleTags])

  const remainingCount = block.tags.length - visibleTags.length
  ...
  if (mode === 'compact') {
    return (
      <button
        ...
        title={`${block.tags.length} tags, ${block.suggestedTags.length} suggestions`}
      >
        <Tags className="w-3 h-3" />
        {hasTags && <span>{block.tags.length}</span>}
        {hasSuggested && (
          <span className="text-amber-500">+{block.suggestedTags.length}</span>
        )}
      </button>
    )
  }
            ...
            {hasSuggested && (
              <div className="mb-3">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-[9px] font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">
                    Suggestions ({block.suggestedTags.length})
                  </span>
                </div>
                <div className="space-y-1">
...
export function InlineBlockTags({
  ...
  const hasContent = block.tags.length > 0 || block.suggestedTags.length > 0
...
export function BlockTagsGutterIndicator({
  ...
  const suggestedCount = block.suggestedTags.length
