 ...
 * 
 * @remarks
 * Weaves are top-level knowledge universes in the FABRIC Codex schema.
...
import DynamicIcon, { isValidIconName } from './DynamicIcon'
...
export default function WeaveCard({
  ...
  return (
    <motion.div
      ...
      className="relative"
    >
      {/* Main Card - With custom styling support */}
      <motion.div
        className={`
          ...
          ${isActive 
            ? `border-${palette.accent}-400 dark:border-${palette.accent}-600 shadow-md shadow-${palette.accent}-500/10` 
      ...
      >
        ...
        <div className="relative p-1.5 z-10">
          {/* Header Row - Clickable to toggle - Ultra Compact */}
          <div
            onClick={onToggle}
            ...
            <div className="flex items-center gap-1">
              {onEdit && (
                <motion.button
                  ...
                  className={`p-1 rounded hover:opacity-90 ${customStyle?.backgroundImage ? 'text-white/60 hover:text-white' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}
                  whileTap={{ scale: 0.95 }}
              ...
              <motion.button
                ...
                className={`px-1.5 py-0.5 rounded text-[9px] font-semibold text-white hover:opacity-90 ${!customStyle?.accentColor ? `bg-gradient-to-r ${palette.gradient}` : ''}`}
                style={customStyle?.accentColor ? { background: accentGradient } : undefined}
                whileTap={{ scale: 0.95 }}
              >
                Explore
              ...
              >
                <ChevronRight className={`w-3 h-3 ${customStyle?.backgroundImage ? 'text-white/60' : 'text-zinc-400'}`} />
              </motion.div>
            </div>
          </div>
        </div>
