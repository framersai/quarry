...
import { GenerationProgress, calculateOverallProgress, type GenerationStage } from './GenerationProgress'
...
function TermCard({
  ...
  const handleCardClick = () => {
    ...
    if (!isEditingTerm && !isEditingDefinition) {
      onToggle()
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {/* Search */}
        <div className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl border ${isDark ? 'bg-zinc-800/50 border-zinc-700' : 'bg-white border-zinc-200'}`}>
        ...
        <div className="relative">
          <select
            ...
            className={`
              appearance-none px-3 py-2 pr-8 rounded-xl border text-sm cursor-pointer
              ${isDark
