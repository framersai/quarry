...
export default function CodeOutput({
  ...
  // Process output
  const { outputLines, isLong, displayedLines } = useMemo(() => {
    const lines = result.output || []
    const isLong = lines.length > collapseThreshold
    const displayedLines = isLong && !isExpanded
      ? lines.slice(0, collapseThreshold)
      : lines
    return { outputLines: lines, isLong, displayedLines }
  }, [result.output, collapseThreshold, isExpanded])

  // Format duration
  const formattedDuration = useMemo(() => {
    if (!result.duration) return null
    if (result.duration < 1000) {
      return `${Math.round(result.duration)}ms`
    }
