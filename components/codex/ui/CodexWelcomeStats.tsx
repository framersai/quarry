 ...
 * @module codex/ui/CodexWelcomeStats
 *
 * @remarks
...
import ProceduralBanner from './ProceduralBanner'
import ActivitySummary from './ActivitySummary'
import InsightFacts from './InsightFacts'

interface CodexWelcomeStatsProps {
  /** Current theme */
  theme?: ThemeName
  /** Total strand count */
  totalStrands: number
 ...
 */
export default function CodexWelcomeStats({
  theme = 'light',
  totalStrands,
  totalWeaves,
  totalLooms,
  ...
  const { greeting, subtitle, emoji } = useMemo(() => 
    getGreetingWithContext({ mood: currentMood ?? undefined }), 
    [currentMood]
  )
  ...
  const handleStartWriting = useCallback((prompt: WritingPrompt) => {
    if (onStartWriting) {
      ...
      window.location.href = `/codex/new?prompt=${encodeURIComponent(prompt.id)}`
    }
  }, [onStartWriting])
