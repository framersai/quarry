...
const LEVEL_THRESHOLDS = [0, 100, 300, 600, 1000, 1500, 2200, 3000, 4000, 5200, 6600, 8200, 10000]

function getLevelFromXP(xp: number): { level: number; progress: number; nextLevelXP: number } {
  for (let i = 0; i < LEVEL_THRESHOLDS.length - 1; i++) {
    if (xp < LEVEL_THRESHOLDS[i + 1]) {
      const currentLevelXP = LEVEL_THRESHOLDS[i]
      const nextLevelXP = LEVEL_THRESHOLDS[i + 1]
      const progress = (xp - currentLevelXP) / (nextLevelXP - currentLevelXP)
      return { level: i + 1, progress, nextLevelXP }
    }
  }
  return { level: LEVEL_THRESHOLDS.length, progress: 1, nextLevelXP: LEVEL_THRESHOLDS[LEVEL_THRESHOLDS.length - 1] }
}
...
export default function LearningProgressDashboard({ 
      ...
      try {
        ...
        if (storedStats) {
          ...
          const defaultStats: LearningStats = {
            ...
            topicMastery: [],
            weeklyActivity: [
              { day: 'Mon', cards: 0, xp: 0 },
              { day: 'Tue', cards: 0, xp: 0 },
              { day: 'Wed', cards: 0, xp: 0 },
              { day: 'Thu', cards: 0, xp: 0 },
              { day: 'Fri', cards: 0, xp: 0 },
              { day: 'Sat', cards: 0, xp: 0 },
              { day: 'Sun', cards: 0, xp: 0 },
            ],
          }
  ...
  const { level, progress, nextLevelXP } = getLevelFromXP(stats.totalXP)
      ...
      <div className={`p-4 rounded-xl mb-6 ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-bold text-emerald-500">{stats.totalXP.toLocaleString()}</p>
          </div>
        </div>
...
export function addXP(amount: number) {
  try {
    ...
    stats.lastStudied = new Date().toLocaleDateString()
    ...
    if (dayIndex !== -1) {
      stats.weeklyActivity[dayIndex].xp += amount
    }
