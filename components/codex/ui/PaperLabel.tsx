...
export default function PaperLabel({
  ...
  const generateAsciiCard = useCallback(() => {
    ...
    // Footer
    lines.push('╠' + hr + '╣')
    lines.push('║  ' + pad(`OpenStrand Schema • FABRIC Codex`) + '║')
    lines.push('╚' + '═'.repeat(w - 2) + '╝')
...
Generated by FABRIC Codex • OpenStrand Schema
`)
  ...
  const getMarkdownExport = () => {
    const lines = [
      ...
      '',
      '---',
      '*OpenStrand Schema • FABRIC Codex*',
    ].filter(Boolean)
    return lines.join('\n')
  }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.setProperties({
        ...
        author: 'FABRIC Codex',
        keywords: tags.join(', '),
        creator: 'FABRIC Codex Paper Model Export'
      })
