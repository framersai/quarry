...
export default function QuarryBrand({
  ...
  // Determine dark mode from theme or system
  const isDark = themeProp?.includes('dark') ?? false
    ...
    >
      ...
      <div className={compact ? 'hidden sm:flex flex-col' : 'flex flex-col'}>
        <div className="flex items-baseline gap-1">
          ...
          {/* Codex Suffix - customizable name with custom color */}
          {showSuffix && (
            <span 
              className={`${classes.codex} font-semibold tracking-[0.08em] uppercase`}
              style={{ 
