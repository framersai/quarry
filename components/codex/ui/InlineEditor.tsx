...
export default function InlineEditor({
  ...
  const saveScrollPositions = useCallback(() => {
    if (textareaRef.current) {
      scrollPositionRef.current.editor = textareaRef.current.scrollTop
    }
  ...
  const restoreScrollPositions = useCallback(() => {
    requestAnimationFrame(() => {
      if (textareaRef.current) {
        textareaRef.current.scrollTop = scrollPositionRef.current.editor
      }
      if (previewRef.current) {
  ...
  return (
    ...
    >
      ...
      <div className={`
        ...
        ${isFullscreen ? 'h-[calc(100vh-100px)]' : 'min-h-[200px] max-h-[50vh]'}
      `}>
        {/* Editor Pane */}
        {(layout === 'editor-only' || layout === 'split-horizontal' || layout === 'split-vertical') && (
          <div className={`
            ${layout === 'split-horizontal' ? 'w-1/2 border-r' : ''}
            ${layout === 'split-vertical' ? 'h-1/2 border-b' : ''}
            ${layout === 'editor-only' ? 'w-full' : ''}
          ...
          `}>
            <textarea
              ...
              className={`
                w-full h-full p-3 resize-none outline-none
                font-mono text-sm leading-relaxed
                ${isFullscreen && layout === 'editor-only' ? 'min-h-[calc(100vh-150px)]' : 'min-h-[180px]'}
                ${isDark 
