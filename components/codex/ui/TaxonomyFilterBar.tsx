...
export default function TaxonomyFilterBar({
  ...
  const tabs: { key: TabType; label: string; icon: React.ComponentType<{ className?: string }>; color: string; available: string[]; selected: string[]; onChange?: (items: string[]) => void; isBlockLevel?: boolean }[] = [
    { key: 'subjects', label: 'Subjects', icon: Folder, color: 'purple', available: availableSubjects, selected: selectedSubjects, onChange: onSubjectsChange },
    { key: 'topics', label: 'Topics', icon: BookOpen, color: 'amber', available: availableTopics, selected: selectedTopics, onChange: onTopicsChange },
    { key: 'tags', label: 'Tags', icon: Tag, color: 'cyan', available: availableTags, selected: selectedTags, onChange: onTagsChange },
  ]
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        `}
      >
        <Filter className={`w-3.5 h-3.5 flex-shrink-0 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`} />
        ...
        <div className="flex-1 flex items-center gap-1.5 min-w-0 overflow-hidden">
          {totalFilters > 0 ? (
            ...
            <span className={`text-[11px] truncate ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
              Filter by subjects, topics, tags, date...
            </span>
          )}
        </div>
