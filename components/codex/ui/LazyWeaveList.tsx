  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${isActive 
          ? isDark ? 'bg-cyan-900/30 border border-cyan-700' : 'bg-cyan-50 border border-cyan-200'
    ...
    >
      ...
      {/* Icon */}
      <Layers className={`w-3.5 h-3.5 ${isActive ? 'text-cyan-500' : 'text-purple-500'}`} />
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-zinc-500">
        <span title={`${loomCount} looms`}>{loomCount}L</span>
        <span>·</span>
        <span title={`${strandCount} strands`}>{strandCount}S</span>
      </div>
    </motion.div>
  )
}
