...
export default function ExecutableCodeBlock({
  ...
  const detectedLanguage = useMemo(() => {
    ...
    try {
      const result = lowlight.highlightAuto(normalizedCode)
      if (result.data && (result.data as { language?: string }).language) {
    ...
    return ''
  }, [normalizedCode, selectedLanguage])
  ...
  const highlightedHtml = useMemo(() => {
    ...
    try {
      ...
      if (detectedLanguage && detectedLanguage !== 'plaintext') {
        try {
          result = lowlight.highlight(detectedLanguage, normalizedCode)
        } catch {
          result = lowlight.highlightAuto(normalizedCode)
        }
      } else {
        result = lowlight.highlightAuto(normalizedCode)
      }
  ...
  }, [normalizedCode, detectedLanguage])
