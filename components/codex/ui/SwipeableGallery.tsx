...
export default function SwipeableGallery({
  ...
  const goToIndex = useCallback(
    (index: number) => {
      if (index < 0 || index >= illustrations.length || isAnimating) return
      setIsAnimating(true)
      setCurrentIndex(index)
      onIndexChange?.(index)
      setTimeout(() => setIsAnimating(false), 300)
    },
    [illustrations.length, onIndexChange, isAnimating]
  )
  ...
  }, [currentIndex, illustrations.length, goToIndex])
  ...
  const handleTouchEnd = useCallback(() => {
    if (Math.abs(touchDeltaX.current) > SWIPE_THRESHOLD) {
      if (touchDeltaX.current > 0 && currentIndex > 0) {
        goPrev()
      } else if (touchDeltaX.current < 0 && currentIndex < illustrations.length - 1) {
        goNext()
      }
    }
    touchDeltaX.current = 0
  }, [currentIndex, illustrations.length, goNext, goPrev])
  ...
  }, [autoAdvance, illustrations.length])
