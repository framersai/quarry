...
import {
  X, Search, Tag, BookOpen, Filter, Check, CheckSquare, Square,
  Layers, GraduationCap, Route, ChevronDown, ChevronRight,
  Sparkles, Target, Hash, Folder, FileText, Loader2, Info
} from 'lucide-react'
...
export default function MultiStrandPicker({
  ...
  const groupedStrands = useMemo(() => {
    ...
    for (const strand of filteredStrands) {
      ...
      groups.get(folder)!.push(strand)
    }

    // Sort folders and strands within
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {groupedStrands.length === 0 ? (
                ...
                groupedStrands.map(({ folder, strands: folderStrands }) => (
                  <div key={folder} className="space-y-1">
                    {/* Folder header */}
                    <div className={`flex items-center gap-2 px-2 py-1 ${textSecondary} text-sm`}>
                      <Folder className="w-4 h-4" />
                      <span className="font-medium">{folder}</span>
                      <span className="text-xs">({folderStrands.length})</span>
                    </div>

                    {/* Strands */}
                    <div className="space-y-1 pl-2">
                      {folderStrands.map(strand => {
