 ...
 *
 * Defines the deployment modes for Quarry:
 * - static: Free open-source community edition
 * - offline: Paid edition with full offline support (requires license)
 *
 * Quarry is the open-source community edition of the Frame.dev knowledge
 * management system. Premium features require a license upgrade.
 *
...
export const COMMUNITY_DEFAULTS: FeatureFlags = {
  ...
  enableSemanticSearch: false, // Disabled in community - uses lexical/BM25 search only
 ...
 * Defaults to 'community' for Quarry open-source edition
 */
export function getEdition(): Edition {
  const edition = process.env.NEXT_PUBLIC_EDITION
  if (edition === 'premium') return 'premium'
  return 'community'
}
