 ...
 * 
 * Allows users to customize their Fabric instance name.
 * Examples: "Fabric Notes", "Fabric Health", "Fabric School", "My Knowledge Base"
 * 
 ...
 * 4. Default: "Fabric"
 * 
 * The full branding is: "[Instance Name] Codex"
 * e.g., "Quarry Codex", "My Notes Codex", "Research Codex"
 * 
...
export interface InstanceConfig {
  /** The app suffix name (e.g., "Codex", "Garden", "Notes", "Library") - "Fabric" brand is immutable */
  codexName: string
...
/** Preset suffix names for quick selection - "Fabric" brand is always constant */
...
export function InstanceConfigProvider({ children }: { children: React.ReactNode }) {
  ...
  // "Fabric" is always the brand prefix, codexName is the customizable suffix
  const displayName = config.showCodexSuffix 
    ? `Fabric ${config.codexName}`
    : 'Fabric'
...
export function getDisplayName(): string {
  ...
  return config.showCodexSuffix ? `Fabric ${config.codexName}` : 'Fabric'
}
