/**
 * NLP Utilities for FABRIC Codex
 * @module lib/nlp
 * 
...
export function extractKeywords(content: string, limit = 10): Array<{ word: string; score: number }> {
  ...
  if (compromise) {
    ...
    doc.verbs().toInfinitive().forEach((v: { text: () => string }) => {
      ...
      if (verb.length > 3 && !STOP_WORDS.has(verb)) {
 ...
 */
export function generateBlockSummaries(content: string, maxLengthPerBlock = 150): BlockSummaryResult[] {
  ...
  return blocks
    ...
    .map(block => ({
      ...
      extractive: generateBlockExtractiveSummary(block, maxLengthPerBlock),
    }))
}
