 ...
 * Uses CodexPageLayout for consistent navigation with sidebar
 * @module codex/browse
 */
...
import { API_ENDPOINTS, REPO_CONFIG } from '@/components/codex/constants'
...
function parseTagsFromEntry(tags: string | string[] | undefined): string[] {
  ...
  if (Array.isArray(tags)) {
    // Flatten in case any array items are comma-separated
    return tags.flatMap(t => String(t).split(',').map(s => s.trim().toLowerCase()).filter(Boolean))
  }
      ...
      {filterValue && (
        <div className={`p-3 rounded-xl border ${isDark ? 'bg-zinc-800/50 border-zinc-700' : 'bg-white border-zinc-200'}`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={() => router.push('/codex/browse')}
              ...
              >
                <div className={`p-2 space-y-1 ${isDark ? 'bg-zinc-900/30' : 'bg-white'}`}>
                  {allSubjects.map(({ name, count }) => (
                    <button
                      key={name}
                      onClick={() => router.push(`/codex/browse?subject=${encodeURIComponent(name)}`)}
                      className={`
              ...
              >
                <div className={`p-2 space-y-1 ${isDark ? 'bg-zinc-900/30' : 'bg-white'}`}>
                  {allTopics.map(({ name, count }) => (
                    <button
                      key={name}
                      onClick={() => router.push(`/codex/browse?topic=${encodeURIComponent(name)}`)}
                      className={`
                        w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-sm
                        transition-colors
                        ${topic === name
                ...
                <div className={`p-2 ${isDark ? 'bg-zinc-900/30' : 'bg-white'}`}>
                  <div className="flex flex-wrap gap-1.5">
                    {allTags.slice(0, 30).map(({ name, count }) => (
                      <button
                        key={name}
                        onClick={() => router.push(`/codex/browse?tag=${encodeURIComponent(name)}`)}
                        className={`
            ...
            >
              <AnimatePresence mode="popLayout">
                {displayedStrands.map((strand) => (
                  ...
                  >
                    <Link
                      href={`/codex/${strand.path.replace(/\.md$/, '')}`}
                      className={`
